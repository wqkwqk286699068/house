...
(function($) {
  ...
  $.fn.extend({
    kslide: function(options) {
        ...
        var defaults = {
            width : 'auto',         //设置宽度
            time : 4,               //kslide-item下一个滚动间隔时间
            slideTime: 0.5,         //kslide-item滚动动画时间
            backDelay:0.5,          //还原位置时间
            controlItemWidth:40,    //控制蓝条宽度
            controlItemGap:5,       //控制蓝条间距
            reverse:false,          //滚动方向，默认从右到左
            height : '',            //设置kslide-wrap高度
            autoHeight:true,        //自动生成kslide-wrap高度
        ...
        if(kslideLength>1){
            ...
            $.each(kslide.find('.kslide-item'),function(i,kslideItem) {
                ...
                if(i==0){
                    ...
                    $(kslideItem).css('transform','translateX('+(o.reverse?'-100%':'100%')+')');
                }
            ...
            kslideInterval = setInterval(function() {
                ...
                kslide.find('.kslide-item').eq(kslideIndex%kslideLength).css('transform','translateX('+(o.reverse?'100%':'-100%')+')');
